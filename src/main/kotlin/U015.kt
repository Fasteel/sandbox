 ...
 * elements, with the same multiplicities (the multiplicity of a member
 * is the number of times it appears). "Same" means, here, that the
 ...
 * comp(a,b) returns false because in b 132 is not the square of
 * any number of a.
 *
 * a = [121, 144, 19, 161, 19, 144, 19, 11]
 ...
 * (except in C++, COBOL, Crystal, D, Dart, Elixir, Fortran, F#,
 * Haskell, Nim, OCaml, Pascal, Perl, PowerShell, Prolog, PureScript,
...
// FIXME: Use map and sorted
fun comp(a: IntArray?, b: IntArray?): Boolean {
    if (a == null || b == null) return false
    val aso = a.sorted()
    val bso = b.sorted()
    for ((i, v) in aso.withIndex()) {
        if (v * v != bso[i]) {
            return false
        }
    }
