 ...
 * N where the sum of the integers to the left of N is equal to the sum of the integers to the
 * right of N.
 *
 ...
 * left side of the index ({1,2,3}) and the sum of the right side of the index ({3,2,1})
 * both equal 6.
 *
...
// FIXME avoid slicing for performance reasons (use foldIndexed)
fun findEvenIndex(arr: IntArray): Int = arr.withIndex().find { v ->
    val leftPart = arr.slice(0..v.index).sum()
    val rightPart = arr.slice(v.index..arr.lastIndex).sum()
    leftPart == rightPart
}?.index ?: -1
