 ...
 * briefly stand, yell, and raise their arms. Immediately upon stretching to full height,
 ...
 * seated in a contiguous circuit all the way around the sport field, and so the wave is able to
 * travel continuously around the arena; in discontiguous seating arrangements, the wave can
 * instead reflect back and forth through the crowd. When the gap in seating is narrow, the wave
 * can sometimes pass through it. Usually only one wave crest will be present at any given time in
...
// FIXME: Use take / drop / filter
fun wave(str: String): List<String> {
    val res = mutableListOf<String>()

    repeat(str.length) { i ->
        var applied = false
        val v = str.toCharArray().mapIndexed { idx, c ->
            if (i == idx && c.isLetter()) {
                applied = true
                c.uppercase()
            } else c
